...
def get_runs():
    with urllib.request.urlopen(TFB_URL) as res:
        data = res.read()
    return data
...
def get_run_meta(runid):
    with urllib.request.urlopen(TFB_URL + f"/results/{runid}") as res:
        data = res.read()
    ...
    with urllib.request.urlopen(TFB_URL + f"{target}/results") as res:
        data = res.read()
...
def get_run_results(path, fw):
    ...
    for bench in ["json", "plaintext"]:
        try:
            with urllib.request.urlopen(TFB_URL + f"{path}/{fw}/{bench}/raw.txt") as res:
