...
class MPServer(AbstractServer[WorkerProcess]):
    ...
    _default_spawners = {
        Interfaces.ASGI: {
            PyRuntimes.asyncio: (_spawn_asgi_asyncio_lifespan_worker, None),
        },
        Interfaces.ASGINL: {PyRuntimes.asyncio: (_spawn_asgi_asyncio_worker, None)},
        Interfaces.RSGI: {
    ...
    def _spawn_worker(self, idx, target, callback_loader) -> WorkerProcess:
        return WorkerProcess(
            ...
            args=(
                ...
                self.loop if self.self.pyruntime == PyRuntimes.asyncio else None,
                self.log_enabled,
                self.log_level,
                self.log_config,
                self.env_files,
