...
pub(crate) fn match_static_file(config: &StaticFilesConfig, req: &HTTPRequest) -> Option<Result<(String, Encoding)>> {
    ...
    if let Some(file_path) = decoded_uri_path.strip_prefix(&config.prefix) {
        ...
        match Path::new(&fpath).canonicalize() {
            Ok(full_path) => {
                ...
                if full_path.starts_with(mount_point) {
                    #[cfg(not(windows))]
                    let base_path = full_path.to_str().map(ToOwned::to_owned);
                    #[cfg(windows)]
                    let base_path = Some(full_path.to_owned());
