...
#[pymethods]
impl RSGI2HTTPProtocol {
    fn read(&self, cb_ok: Py<PyAny>, cb_err: Py<PyAny>) -> Option<super::callbacks::PyAbortHandle> {
        if let Some(rx) = self.body.lock().unwrap().take() {
            let rt = self.rt.clone();
            let task = self.rt.spawn(async move {
                match rx.collect().await {
                    Ok(data) => rt.spawn_blocking_loopback(move |py| {
                        _ = cb_ok.call1(py, (data.to_bytes(), true));
                    }),
                    _ => rt.spawn_blocking_loopback(move |py| {
                        _ = cb_err.call1(
                            py,
                            (crate::rsgi::errors::RSGIProtocolClosed::new_err(
                                "RSGI transport is closed",
                            ),),
                        );
                    }),
                }
